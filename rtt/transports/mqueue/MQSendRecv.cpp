                             ...
                             bool is_sender)
{
    ...
    {
        int the_error = errno;
        Logger::log().logf(Logger::Error, "MQSendRecv",
                           "FAILED opening '%s' with message size %ld, buffer size %ld for %s",
                           policy.name_id.c_str(), static_cast<long>(mattr.mq_msgsize),
                           static_cast<long>(mattr.mq_maxmsg),
                           is_sender ? "writing :" : "reading :");
        ...
        case EMFILE:
            ...
            break;
        case ENAMETOOLONG:
            Logger::log().logf(Logger::Error, "MQSendRecv", "Name was too long.");
            break;
        case ENFILE:
            Logger::log().logf(Logger::Error, "MQSendRecv",
        ...
        case ENOSPC:
            ...
            break;
        case ENOMEM:
            Logger::log().logf(Logger::Error, "MQSendRecv", "Insufficient memory.");
            break;
        default:
            Logger::log().logf(Logger::Error, "MQSendRecv",
                               "Submit a bug report. An unexpected mq error occured with errno=%d: %s",
                               errno, strerror(errno));
        }
        throw std::runtime_error("Could not open message queue: mq_open returned -1.");
    }

    Logger::log().logf(Logger::Debug, "MQSendRecv",
                       "Opened '%s' with mqdes='%d', msg size='%ld' an queue length='%ld' for %s",
                       policy.name_id.c_str(), static_cast<int>(mqdes),
                       static_cast<long>(mattr.mq_msgsize),
                       static_cast<long>(mattr.mq_maxmsg),
                       is_sender ? "writing." : "reading.");
...
bool MQSendRecv::mqReady(base::DataSourceBase::shared_ptr ds, base::ChannelElementBase* chan)
{
    ...
    if (!mis_sender)
    {
        ...
        {
            Logger::log().logf(Logger::Error, "MQSendRecv",
                               "Failed to receive initial data sample for MQ Channel Element: %s",
                               strerror(errno));
            return false;
        }
    }
...
bool MQSendRecv::mqWrite(RTT::base::DataSourceBase::shared_ptr ds)
{
    ...
    if (blob.first == 0)
    {
        Logger::log().logf(Logger::Error, "MQSendRecv", "MQChannel: failed to marshal sample");
        return false;
    }
    ...
    if (mq_send(mqdes, lbuf, blob.second, 0) == -1)
    {
        ...
        Logger::log().logf(Logger::Error, "MQSendRecv",
                           "MQChannel %d became invalid (mq length=%d, msg length=%d): %s",
                           static_cast<int>(mqdes), max_size, blob.second, strerror(errno));
        return false;
    }
    return true;
}
